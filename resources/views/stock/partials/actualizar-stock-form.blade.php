<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            {{ __('Actualizar Dados do Produto') }}
        </h2>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex flex-wrap -mx-3">
            <div class="w-full sm:w-1/2 px-3 py-3">
                <x-input-label for="name" :value="__('Nome do Fornecedor:')" />
                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required autofocus autocomplete="name" />
                <x-input-error class="mt-2" :messages="$errors->get('name')" />
            </div>

            <div class="w-full sm:w-1/2 px-3 py-3">
                <x-input-label for="email" :value="__('Medicamento:')" />
            ...
            <div class="w-full sm:w-1/2 px-3 py-3">
                ...
                <x-text-input id="composicao" name="composicao" type="text" class="mt-1 block w-full read-only:true" :value="old('composicao')" required autofocus autocomplete="composicao" />
                <x-input-error class="mt-2" :messages="$errors->get('cell')" />
            </div>

            <div class="w-full sm:w-1/2 px-3 py-3">
                <x-input-label for="nuit" :value="__('Unidade de medida:')" />
            ...
            <div class="w-full sm:w-1/2 px-3 py-3">
                <x-input-label for="nuit" :value="__('Quantidade:')" />
            ...
            <div class="w-full sm:w-1/2 px-3 py-3">
                ...
                <x-text-input id="preco" name="preco" type="text" class="mt-1 block w-full" :value="old('preco')" required autofocus autocomplete="nuit" />
                <x-input-error class="mt-2" :messages="$errors->get('endereco')" />
            </div>

            <div class="w-full sm:w-1/2 px-3 py-3">
                <x-input-label for="validade" :value="__('Validade:')" />
                <x-text-input id="validade" name="validade" type="date" class="mt-1 block w-full" :value="old('endereco')" required autofocus autocomplete="nuit" />
                <x-input-error class="mt-2" :messages="$errors->get('endereco')" />
            </div>
        </div>
