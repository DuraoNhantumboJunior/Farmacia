<div class="relative overflow-x-auto">
    ...
    <div class="max-h-64 overflow-y-auto">
        <table class="min-w-full w-full">
            <thead class="bg-gray-400 text-white">
                <tr>
                    <th class="px-10 py-3 mx-16 text-left text-xs font-medium uppercase tracking-wider ">Nome</th>
                    <th class="px-15 py-3 mx-16 text-left text-xs font-medium uppercase tracking-wider ">Endereço</th>
                    <th class="px-15 py-3 mx-16 text-left text-xs font-medium uppercase tracking-wider ">Telefone</th>
                    <th class="px-15 py-3 mx-16 text-left text-xs font-medium uppercase tracking-wider ">Email</th>
                    <th class="px-15 py-3 mx-16 text-left text-xs font-medium uppercase tracking-wider ">NUIT</th>
                    <th class="px-15 py-3 mx-16 text-left text-xs font-medium uppercase tracking-wider">Operações</th>
                </tr>
            </thead>
            <tbody id="fornecedorTable" class="bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200">
                @foreach($fornecedores as $fornecedor)
                <tr class="border-t">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">{{$fornecedor->nome}}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{$fornecedor->endereco}}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{$fornecedor->telefone}}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{$fornecedor->email}}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{$fornecedor->nuit}}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="actualizar-fornecedor/{{$fornecedor->id}}">
    ...
    <script>
        document.getElementById('searchInput').addEventListener('keyup', function() {
            ...
            const rows = document.querySelectorAll('#fornecedorTable tr');
